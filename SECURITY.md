...
We actively support shape-xml with security updates:
...
We take security vulnerabilities seriously. If you discover a security issue in shape-xml, please report it responsibly.
...
1. **GitHub Private Vulnerability Reporting** (Preferred)
   - Navigate to the [Security tab](https://github.com/shapestone/shape-xml/security) of this repository
   - Click "Report a vulnerability"
   - Fill out the vulnerability report form

2. **Email**
   - Send details to: security@shapestone.com
   - Include "shape-xml Security" in the subject line
...
shape-xml is a parser library that processes untrusted input. Be aware of these potential security considerations:

1. **Denial of Service (DoS)**
   - Malformed XML with deeply nested elements
   - Extremely large files or payloads
   - XML bombs (billion laughs attack, quadratic blowup)
   - Recursive entity expansion

2. **Resource Exhaustion**
   - Memory consumption from large ASTs
   - CPU consumption from complex XML structures

3. **XML-Specific Vulnerabilities**
   - External entity injection (XXE) - Note: shape-xml does not process external entities by default
...
4. **Input Validation**
   ...
   - Be cautious with user-controlled XML

### Best Practices

When using shape-xml in production:
...
- **Disable External Entities**: If you add external entity processing, ensure it's disabled for untrusted input
...
If you have questions about this security policy or shape-xml's security posture, please open a public discussion in GitHub Discussions or contact security@shapestone.com.

---

Thank you for helping keep shape-xml and its users safe!
